...
class ReportService {
  ...
  // ✅ 1. إذا كان البلاغ على مستخدم - نعلق المستخدم
  if (targetType === 'user') {
  ...
  // ✅ 2. إذا كان البلاغ على فعالية - نحظر الفعالية
  if (targetType === 'event') {
  ...
  // ✅ 3. إذا كان البلاغ على عمل فني - نحظر العمل الفني
  if (targetType === 'artwork') {
  ...
  // ✅ 4. إذا كان البلاغ على تعليق - نعلق صاحب التعليق
  if (targetType === 'comment' && commentOwnerId) {
    ...
    // ✅ 5. التحقق من إجمالي بلاغات تعليقات العمل الفني
    if (artworkId) {
  ...
  try {
    ...
    // جلب جميع البلاغات على هذا العمل الفني
    ...
    // إذا وصل للحد المطلوب (10 بلاغات)
    if (uniqueCount >= 10) {
  ...
  try {
    ...
    // إذا وصل للحد المطلوب (10 بلاغات من أشخاص مختلفين)
    if (uniqueCount >= 10) {
  ...
  async getAllReports(filters = {}) {
    ...
    // إضافة معلومات إضافية عن المستهدف
 ...
 async resolveReport(reportId, adminId, action, adminNote = '', actionTaken = null) {
  ...
  // إذا تم قبول البلاغ (يعني البلاغ صحيح)
  if (action === 'resolve') {
    ...
    if (report.targetType === 'artwork') {
      // ✅ عند قبول البلاغ (يعني البلاغ صحيح) - نحظر العمل الفني، لا نحذفه
      const artwork = await Artwork.findById(report.targetId);
      if (artwork && artwork.status !== 'blocked') {
        artwork.status = 'blocked';
        await artwork.save();
