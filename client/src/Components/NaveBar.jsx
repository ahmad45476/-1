...
import { Home, Compass, User, CreditCard, LogOut, Calendar } from "react-feather"; // ✅ أضفنا Calendar
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    ...(!token
      ...
      : [
          ...
          { name: "تسجيل الخروج", onClick: logout, icon: <LogOut size={18} /> },
        ]),
  ];
  ...
  const handleWalletClick = async () => {
    ...
    if (isConnected) {
      if (window.confirm("هل تريد قطع الاتصال بالمحفظة؟")) {
        disconnectWallet();
      }
      return;
    }
                  ...
                  <div className="absolute left-0 right-0 top-full mt-2 flex flex-col gap-1 min-w-[220px]">
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        <span className="opacity-75">العنوان:</span>
                      ...
                      <div className="flex justify-between items-center text-[10px] opacity-75">
                        ...
                        <span className="font-mono font-medium">
                          {networkName === "شبكة 31337" ? "Hardhat Local" : networkName}
                        </span>
                      </div>
                    </div>
                  </div>
