...
const LatestArtworks = () => {
  ...
  const navigate = useNavigate(); // ⬅️ لإضافة التنقل
  ...
  const fetchTopArtworks = async () => {
    try {
      ...
      // استخدم نفس الـAPI الذي تستخدمه في Explore.jsx
      ...
      // فرز الأعمال حسب عدد الإعجابات (الأكثر إعجاباً أولاً)
      const sortedArtworks = artworksData
        ...
        .map(artwork => ({
          ...
          // حساب عدد الإعجابات من بيانات مختلفة
        ...
        .sort((a, b) => b.likesCount - a.likesCount) // ترتيب تنازلي حسب الإعجابات
        .slice(0, 3); // أخذ أول 3 أعمال (الأكثر إعجاباً)
  ...
  const handleLike = async (artworkId, e) => {
    e.stopPropagation(); // ⬅️ لمنع فتح صفحة العمل عند الإعجاب
    ...
    try {
      ...
      if (response.data.success) {
        // تحديث عدد الإعجابات في الواجهة وإعادة الفرز
  ...
  const getArtworkImageUrl = (artwork) => {
    ...
    // إذا كان الرابط يبدأ بـ /uploads أضف العنوان الأساسي
  ...
  // دالة للانتقال إلى صفحة العمل الفني
          ...
          {artworks.map((artwork) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => goToArtworkPage(artwork.id)} // ⬅️ عند الضغط على البطاقة
              >
                {/* صورة العمل الفني */}
                {imageUrl ? (
  ...
  <div className="relative w-full h-64 overflow-hidden">
   {imageUrl ? (
  <div className="relative w-full h-80 overflow-hidden"> {/* ⬅️ ارتفاع أكبر */}
