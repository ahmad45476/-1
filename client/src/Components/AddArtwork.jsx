    ...
    try {
      ...
      if (contract && wallet && formData.price > 0) {
        try {
          ...
          // ✅ استخراج tokenId من الـ events (الطريقة الصحيحة)
          try {
            // البحث عن حدث Transfer (وهو حدث ERC721 القياسي)
            ...
            if (transferEvent) {
              ...
              // في حدث Transfer، tokenId هو المعامل الثالث (index 2)
              tokenId = Number(parsedLog?.args?.tokenId || parsedLog?.args[2]);
              console.log('✅ Token ID من حدث Transfer:', tokenId);
            }
            
            // إذا لم يتم العثور على Transfer، ابحث عن حدث ArtworkCreated أو ArtworkMinted
            if (!tokenId) {
